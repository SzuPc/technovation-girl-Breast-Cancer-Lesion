...
Code borrowed from mmdet.core.evaluation.mean_ap
"""
...
def get_cls_results(det_results, annotations, class_id, maxDets=None):
    ...
    Args:
        ...
        maxDets (int|None): maximum number of detections per image, Default(None)
    ...
    if maxDets is None:
        ...
        cls_dets = [img_res[class_id][:maxDets, :] for img_res in det_results]
    cls_gts = []
    cls_gts_ignore = []
...
def eval_map(
    ...
    for i in range(num_classes):
        ...
        rec = recalls[0, :]
        ret_thres.append(np.interp(fixed_recall, rec, probs))
        # calculate AP
    ...
    assert ret_thres, "num_classes == 0!!"
